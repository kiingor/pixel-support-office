-- Agents table
-- MIGRATION: If updating an existing Supabase instance, run:
--   ALTER TABLE agents DROP CONSTRAINT IF EXISTS agents_type_check;
--   ALTER TABLE agents ADD CONSTRAINT agents_type_check
--     CHECK (type IN ('suporte', 'qa', 'qa_manager', 'dev', 'dev_lead', 'log_analyzer', 'ceo'));
CREATE TABLE IF NOT EXISTS agents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('suporte', 'qa', 'qa_manager', 'dev', 'dev_lead', 'log_analyzer', 'ceo')),
...
-- Agent skill evolution (learnings accumulated per task)
-- MIGRATION: CREATE TABLE IF NOT EXISTS agent_learnings (...)
CREATE TABLE IF NOT EXISTS agent_learnings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  agent_name TEXT NOT NULL,
  role TEXT NOT NULL,
  learning TEXT NOT NULL,        -- 1-2 sentence insight generated by AI
  task_context TEXT,             -- brief context of what task triggered this learning
  tasks_completed_at INT,        -- agent's tasks_completed count at the time
  created_at TIMESTAMPTZ DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_agent_learnings_name ON agent_learnings(agent_name, created_at DESC);
